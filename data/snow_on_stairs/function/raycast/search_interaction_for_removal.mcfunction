...
execute align xyz positioned ~.5 ~.5 ~.5 \
    if entity @e[tag=snow_on_stairs.snow,distance=..0.1,type=minecraft:interaction] \
    run return run \
    execute as @e[tag=snow_on_stairs.snow,distance=..0.1,type=minecraft:interaction] at @s run function snow_on_stairs:destroy/destroy
